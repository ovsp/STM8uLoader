...
public class Dumps
{
	    ...
	    public static byte Load_First_Dump(byte[] dmp, string str){
	    	...
	    	byte[] dmpToLoad = dmp;
        	byte rvsn = 0;
			Console.WriteLine(" Ждем последовательно байт 0x00 (BREAK) и байт 0x0D версии загрузчика. Нажми кнопку сброса на плате.");
			while(unready)
			{
	    ...
	    public static void Go_Cmnd(byte[] gbt){
	    	...
	    	// необходимо отправить команду загруженному блоку в формате  { GO cmd, go_adrH, go_adrL, SPH, SPL, XH, XL, A }
	    	err_Msg = " OK";
	    	go_bytes = gbt;
	    ...
	    public static void Load_Dump_v0D(byte[] dmp, string str){
	    	// метод вызывается при замене блока в верхних адресах RAM памяти
	    	// необходимо отправить команду загруженному блоку в формате  { GO cmd, go_adrH, go_adrL, SPH, SPL, XH, XL, A }
	    	err_Msg = " OK";
	    	byte[] dmpToLoad = dmp;
	    	go_bytes[0] = 0xF5;	//  GO cmd команда перехода
	    	go_bytes[1] = 0x03;	//  go_adrH 	старший байт требуемого адреса перехода
	    	go_bytes[2] = 0xE8;	//  go_adrL 	младший байт требуемого адреса перехода
	    	go_bytes[3] = 0x03;	//  SPH 	    старший байт регистра указателя стека SP
	    	go_bytes[4] = 0xE7;	//  SPL 	    младший байт регистра указателя стека SP
	    	go_bytes[5] = 0x00;	//  XH 	        старший байт индексного регистра X
	    	go_bytes[6] = 0x00;	//  XL 	        младший байт индексного регистра X
	    	go_bytes[7] = 0x0D;	//  A 	        регистр A
        	...
        	sPort.Write(go_bytes, 0, 1); // передаем команду $F5
				...
				while(queueBytes.Count == 0){}  rx_byte = queueBytes.Dequeue(); 
				        	if ( rx_byte != go_bytes[0] ){ 
        						err_Msg = " Эхо не соответствует команде 0xF5"; 
        						return; }
        	sPort.Write(go_bytes, 1, go_bytes.Length-1); // передаем адреса и регистры для перехода к начальному загрузчику
        ...
        public static SortedDictionary<int, byte> Read_128k_v0D(int adr, int cntr){
        	err_Msg = " OK";
        	...
        	if (crntModul != "Read_128000v0D") Load_Dump_v0D88(Dumps.Read_128000v0D, "Read_128000v0D");
        ...
        public static SortedDictionary<int, byte[]> ReadBlocks_128000(SortedDictionary<int, byte[]> adrBt){
        	...
        	if (crntModul != "ReadBlocks_128000v14_25") Load_Dump_v0D88(Dumps.ReadBlocks_128000v14_25, "ReadBlocks_128000v14_25");
		...
		public static SortedDictionary<int, byte[]> Read_by_Mask128000(SortedDictionary<int, byte[]> stdctnr){
        	...
        	if (crntModul != "Read_128000v14_25") Load_Dump_v0D88(Dumps.Read_128000v14_25, "Read_128000v14_25");	
        ...
        public static void Write_RAM_128000(SortedDictionary<int, byte[]> adrBt){
        	...
        	if (crntModul != "Write_RAM_128000v14_25") Load_Dump_v0D88(Dumps.Write_RAM_128000v14_25, "Write_RAM_128000v14_25");	
        ...
        public static void Copy_RAM_128000(int adrDst, int adrSrc, int cntr){
        	...
        	int cntrCopy  = cntr;
        					if ( dstAdrCopy < 0x0000 | (dstAdrCopy + cntrCopy) > 0x03FF){
		        					...
		        					return;}
        	if (crntModul != "Copy_RAM_128000v14_25") Load_Dump_v0D88(Dumps.Copy_RAM_128000v14_25, "Copy_RAM_128000v14_25");
        ...
        public static void WriteByte_EEPROM_128000(SortedDictionary<int, byte[]> adrBt){
        	...
        	if (crntModul != "WriteByte_EEPROM_128000v14_25") Load_Dump_v0D88(Dumps.WriteByte_EEPROM_128000v14_25, "WriteByte_EEPROM_128000v14_25");
        ...
        public static void WriteWord_EEPROM_128000(SortedDictionary<int, byte[]> adrBt){
        	...
        	if (crntModul != "WriteWord_EEPROM_128000v14_25") Load_Dump_v0D88(Dumps.WriteWord_EEPROM_128000v14_25, "WriteWord_EEPROM_128000v14_25");
        ...
        public static void WriteBlock_EEPROM_128000(SortedDictionary<int, byte[]> adrBt){
			...
			if (crntModul != "WriteBlock_EEPROM_128000v1425") Load_Dump_v0D88(Dumps.WriteBlock_EEPROM_128000v1425, "WriteBlock_EEPROM_128000v1425");
        ...
        public static void WriteBlocks_EEPROM_128000(SortedDictionary<int, byte[]> adrBt){
			...
			if (crntModul != "WriteBlocks_EEPROM_128000v14_25") Load_Dump_v0D88(Dumps.WriteBlocks_EEPROM_128000v14_25, "WriteBlocks_EEPROM_128000v14_25");
        ...
        public static void WriteBytes_OPTION_128000(SortedDictionary<int, byte> adrBt){
			...
			if (crntModul != "WriteBytes_OPTION_128000v1425") Load_Dump_v0D88(Dumps.WriteBytes_OPTION_128000v1425, "WriteBytes_OPTION_128000v1425");
        ...
        public static void WriteByte_FLASH_128000(SortedDictionary<int, byte[]> adrBt){
			...
			if (crntModul != "WriteByte_FLASH_128000v14_25") Load_Dump_v0D88(Dumps.WriteByte_FLASH_128000v14_25, "WriteByte_FLASH_128000v14_25");
        ...
        public static void WriteWord_FLASH_128000(SortedDictionary<int, byte[]> adrBt){
			...
			if (crntModul != "WriteWord_FLASH_128000v14_25") Load_Dump_v0D88(Dumps.WriteWord_FLASH_128000v14_25, "WriteWord_FLASH_128000v14_25");
        ...
        public static void WriteBlocks_FLASH_128k_v0D(SortedDictionary<int, byte[]> adrBt){
        	err_Msg = "OK";
        	SortedDictionary<int, byte[]> AddressByte = new SortedDictionary<int, byte[]>(adrBt);
        	
			if (crntModul != "WriteBlocks_FLASH_128000v0D") Load_Dump_v0D88(Dumps.WriteBlocks_FLASH_128000v0D, "WriteBlocks_FLASH_128000v0D");
	...
	public static SortedDictionary<int, byte[]> FLASH_AdrBlcsSorting_v0D(SortedDictionary<int, byte> sort, SortedDictionary<int, byte> unchng) {
		SortedDictionary<int, byte> sortAdrBt = new SortedDictionary<int, byte>(sort);			// для счетчика foreach
		SortedDictionary<int, byte> unchngAdrBt = new SortedDictionary<int, byte>(unchng);			// для счетчика foreach
	    ...
	    foreach(KeyValuePair<int, byte> kvp in unchngAdrBt){
	    	if (sortAdrBt.ContainsKey(kvp.Key)) {
	    	   	err_Msg = " Имеются адреса кода копировщика"; return FLASH_AdrBlcks;
	    	   }
	        else sortAdrBt.Add(kvp.Key, kvp.Value);
	    }
...
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xE6, 0x03, 0xCC, 0x72, 
...
0x30, 0x52, 0x0B, 0x72, 0x95, 0x31, 0x52, 0xC6, 0xFB, 0x30, 0x52, 0x0B, 0x72, 0xE7, 0x03, 0x31, 
0x52, 0x55, 0xFB, 0x30, 0x52, 0x0B, 0x72, 0xE6, 0x03, 0x31, 0x52, 0x55, 0xFB, 0x30, 0x52, 0x0B, 
...
0xEF, 0x02 };   
   public readonly static byte[] WriteBlock_FLASH_128000v0D = {  };
   public readonly static byte[] WriteBlocks_FLASH_128000v0D = { 0x00, 
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xE6, 0x03, 0xCC, 
...
0xFB, 0x30, 0x52, 0x0B, 0x72, 0x95, 0x31, 0x52, 0xC6, 0xFB, 0x30, 0x52, 0x0B, 0x72, 0xE7, 0x03, 
0x31, 0x52, 0x55, 0xFB, 0x30, 0x52, 0x0B, 0x72, 0xE6, 0x03, 0x31, 0x52, 0x55, 0xFB, 0x30, 0x52, 
...
0x52, 0x01, 0x35, 0x33, 0x52, 0x00, 0x35 };
	...
	public static byte[] boot_FLASH_8000_v0D4088 = {0xAE, 0x40, 0x88, 0x5A, 0xF6, 0x26, 0xFB, 0x81};
	...
	public static byte[] go_bytes = { 0xF5, 0x03, 0xE8, 0x03, 0xE7, 0x00, 0x00, 0x0D}; // {GO cmd, go_adrH, go_adrL, SPH, SPL, XH, XL, A }
